...
export function VideoReviews({ videoSlug }: { videoSlug: string }) {
  ...
  async function loadReviews() {
    const { data } = await supabase
      .from('video_reviews')
      .select('id, video_slug, user_id, rating, body, created_at, updated_at, profiles(username)')
  ...
  async function submitReview(e: React.FormEvent) {
    ...
    const { error: insertError } = await supabase.from('video_reviews').upsert(
      {
    ...
    <section className="mt-14 border-t border-border pt-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 font-serif text-3xl">Ce spun privitorii</h2>
        </div>
      ...
      {user ? (
        <form onSubmit={submitReview} className="mt-6 rounded-lg border border-border p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <p className="text-sm text-muted-foreground">
              Recenzia rămâne pe pagina acestui episod.
            </p>
          </div>
          <textarea
            value={body}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-medium text-foreground">
                    {review.profiles?.username || 'Privitor'}
                  </p>
